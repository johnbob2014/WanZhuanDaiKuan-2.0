...
- (NSArray *)constraintsTopToButtomOfView:(UIView *)view constant:(CGFloat)c
{
    ...
    return [NSLayoutConstraint constraintsWithVisualFormat:@"V:[view]-(distance)-[selfView]" options:0 metrics:@{@"distance":@(c)} views:NSDictionaryOfVariableBindings(view, selfView)];
}

- (NSArray *)constraintsBottomToTopOfView:(UIView *)view constant:(CGFloat)c
{
    UIView *selfView = self;
    return [NSLayoutConstraint constraintsWithVisualFormat:@"V:[selfView]-(distance)-[view]" options:0 metrics:@{@"distance":@(c)} views:NSDictionaryOfVariableBindings(selfView, view)];
}

- (NSArray *)constraintsLeftToRightOfView:(UIView *)view constant:(CGFloat)c
{
    UIView *selfView = self;
    return [NSLayoutConstraint constraintsWithVisualFormat:@"H:[view]-(distance)-[selfView]" options:0 metrics:@{@"distance":@(c)} views:NSDictionaryOfVariableBindings(view, selfView)];
}

- (NSArray *)constraintsRightToLeftOfView:(UIView *)view constant:(CGFloat)c
{
    UIView *selfView = self;
    return [NSLayoutConstraint constraintsWithVisualFormat:@"H:[selfView]-(distance)-[view]" options:0 metrics:@{@"distance":@(c)} views:NSDictionaryOfVariableBindings(selfView, view)];
}
...
//删除单个约束
- (void)removeSingleConstraint:(NSLayoutConstraint *)constraint{
...
//删除多个约束
- (void)removeMultipleConstraints:(NSArray *)constraints{
    for (NSLayoutConstraint *constraint in constraints) {
        for (NSLayoutConstraint *superViewConstraint in self.superview.constraints) {
